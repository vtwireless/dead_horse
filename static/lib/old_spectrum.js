...
*/
	...
	function dataToColor(data){
		...
		/*if(checkIfZeroes(data)){
            ...
            var data_pos_unit =20/data.length;//dividing the width of the grid in the x3d scene by the length of data.
		...
		for (var i=0; i<data.length; i++) {
                   ...
                   // alert(trans_x[0].translation+" "+point_pos);
                    ...
                    if(point_pos >= (trans_x[2].translation - trans_x[2].bandwidth) && point_pos <= (trans_x[2].translation + trans_x[2].bandwidth))
                    {colorArray.push(0+' '+0+ ' '+1);}
                    
                   else if(point_pos >= (trans_x[3].translation - trans_x[3].bandwidth) && point_pos <= (trans_x[3].translation + trans_x[3].bandwidth))
                    {colorArray.push(0+' '+0+ ' '+1);}
                    
                   else if(point_pos >= (trans_x[4].translation - trans_x[4].bandwidth) && point_pos <= (trans_x[4].translation + trans_x[4].bandwidth))
                    {colorArray.push(1+' '+0+ ' '+0);}
                    
                   else if(point_pos >= (trans_x[5].translation - trans_x[5].bandwidth) && point_pos <= (trans_x[5].translation + trans_x[5].bandwidth))
                    {colorArray.push(1+' '+0+ ' '+0);}
                    
                    else if(point_pos >= (trans_x[0].translation - trans_x[0].bandwidth) && point_pos <= (trans_x[0].translation + trans_x[0].bandwidth))
                    {colorArray.push(0+' '+1+ ' '+0);}
                    
                    else if(point_pos >= (trans_x[1].translation - trans_x[1].bandwidth) && point_pos <= (trans_x[1].translation + trans_x[1].bandwidth))
                    {colorArray.push(0+' '+1+ ' '+0);}
			...
			if(i>=low && i<=high) {
				...
				//colorArray.push(HSVtoRGB(c, 0, 0));
                        ...
                        //colorArray.push(data[i]/2+' '+data[i]/2+ ' '+data[i]/2);//HSVtoRGB(c, 1, 1));//Ayat
			//}
	...
	thisModule.initialize = function (lastDigitsIP, nTimeSlices, gridID, gridColorID, heatmapDivID, show_floors, showStatus, transParams, crtsMetricsId) {
		...
		socket.on('initial_params', function(data) {
			console.log(JSON.stringify(data));
                        ...
                        var diffue_color = ['green','blue','red'];// for coloring the boxes
		...
		socket.on('spectrumUpdate', function(data) {
			console.log(JSON.stringify(data));
                ...
                var index_0 = 0,index_1 = 0, index_2 = 0, index_3 = 0, index_A = 0, index_B = 0, throughput_arr_0 = [], throughput_arr_1 = [],throughput_arr_2 = [],throughput_arr_3 = [],throughput_arr_A = [],throughput_arr_B = [];
		/*var throughput_dialog_div = document.createElement('div');
			...
			console.log(JSON.stringify(data));
                        ...
                        if(data.node == 0){throughput_arr_0.push({ node: data.node, value: data.throughput, index: index_0 }); ++index_0;};
                        ...
                        for(var i = 0; i < Math.min(index_0,index_1); i++){
                            
                           throughput_arr_A.push({value: throughput_arr_0[i].value + throughput_arr_1[i].value, index: i });
                           //console.log("A "+" "+i+" "+throughput_arr_A[i].value);
                        }
                        for(var i = 0; i < Math.min(index_2,index_3); i++){
                           throughput_arr_B.push({value: throughput_arr_2[i].value + throughput_arr_3[i].value, index: i });
                    ...
                    function resizeCanvas() {
                         ...
                         var margin = {top: 20, right: 20, bottom: 30, left: 70},
                            width = $("#container").innerWidth()/2 - margin.left - margin.right,
                        ...
                        var y = d3.scale.linear()
                                .range([height, 0]);
                        x.domain(d3.extent(throughput_arr_0, function(d) { return d.index; }));
                        y.domain([0,2000]);//(d3.extent(throughput_arr_0, function(d) { return (d.value/1000); }));
                        ...
                        yA.domain([0,2000]);//d3.extent(throughput_arr_3, function(d) { return (d.value/1000); }));
                        ...
                        yB.domain([0,2000]);//d3.extent(throughput_arr_3, function(d) { return (d.value/1000); }));
                ...
                svg.append("g")
                   ...
                   .append("text")
                      ...
                      .text("node_0_Throughput(kbps)");
                
                var svg1 = d3.select("#right")
                .append("svg")
                    .attr("width", width + margin.left + margin.right)
                    .attr("height", height + margin.top + margin.bottom)
                .append("g")
