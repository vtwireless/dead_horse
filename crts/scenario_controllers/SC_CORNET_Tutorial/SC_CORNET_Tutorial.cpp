...
static inline ssize_t sendn(int fd, const char *buf, size_t len, int flag)
{
    ssize_t ret;
    while(len > 0)
    {
        ret = send(fd, buf, len, flag);
        if(ret <= 0) { printf("Socket send() failed\n"); exit(EXIT_FAILURE); } // We're screwed
        len -= ret;
        buf += ret;
    }
    return (ssize_t) len;
...
static inline ssize_t recvn(int fd, char *buf, size_t len, int flag)
{
    ssize_t ret;
    while(len > 0)
    {
        ret = recv(fd, buf, len, flag);
        if(ret <= 0) { printf("Socket recv() failed\n"); exit(EXIT_FAILURE); } // We're screwed
        len -= ret;
        buf += ret;
    }
    return (ssize_t) len;
...
// From here down does the same thing as in SC_CORNET_Display
