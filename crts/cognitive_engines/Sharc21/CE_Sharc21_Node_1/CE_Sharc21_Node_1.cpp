...
#include "CE_Sharc21_Node_1.hpp"

// constructor
CE_Sharc21_Node_1::CE_Sharc21_Node_1(int argc, char **argv, ExtensibleCognitiveRadio *_ECR) {
...
// destructor
CE_Sharc21_Node_1::~CE_Sharc21_Node_1() {
  // Destroy timers
...
// execute function
void CE_Sharc21_Node_1::execute() {
  ...
  if (!stop_tx) {
    ...
    // Set Node 2's control info to default value.
    ...
    node2_control_info_default_set = true;
    
  }
  
  // Listen on the starting frequencies to see if anybody is already there.
  // Change flag name
...
void CE_Sharc21_Node_1::CountErrors(ExtensibleCognitiveRadio *ECR) {
...
void CE_Sharc21_Node_1::CheckDataRate(ExtensibleCognitiveRadio *ECR, int frames_rcvd, int errors_rcvd, int correct_rcvd, int total_bytes_rcvd, int total_correct_bytes_rcvd, float time_passed) {
  ...
  */
  ...
  // If % of valid payloads >= 95% for two consecutive periods, change FEC to increase rate. If % of valid payloads < 95%, change FEC accordingly.
  if (error_percentage >= 0.2) {
    ...
    if (ECR->get_tx_fec0() == LIQUID_FEC_RS_M8) {
      ...
      float tx_gain_step = 1.0;
      
      // Tell Node 2 to reset its FEC0.
      ...
      node2_control_info_default_set = false;
...
void CE_Sharc21_Node_1::ResetTheDangControlInfo(ExtensibleCognitiveRadio *ECR) {
  
  if (current_tx_control_info[0] != 'r') {
    
    // Reset Node 2's conrol info to 'r'
    current_tx_control_info[0] = 'r';
    std::memset(&current_tx_control_info[1], 0, 5);
    ECR->set_tx_control_info(current_tx_control_info);
    node2_control_info_default_set = true;
...
void CE_Sharc21_Node_1::SendIntentToChangeTheDangFreq(ExtensibleCognitiveRadio *ECR) {
	
	float new_node2_tx_freq = rx_freqs[i];
...
void CE_Sharc21_Node_1::ChangeTheDangFreq(ExtensibleCognitiveRadio *ECR) {
  ...
  // Tell Node 2 to set stop sending the frequency change.
...
void CE_Sharc21_Node_1::RateCrap(ExtensibleCognitiveRadio *ECR) {
  // ***** Rate Stuff *****
  ...
  if (frame_flag == 2) {   
    ...
    // Calculate the frame error percentage and valid payload percentage.
...
void CE_Sharc21_Node_1::ControlInfoCrap(ExtensibleCognitiveRadio *ECR) {
  // Control Info Stuff
  ...
  if ((char)ECR->CE_metrics.control_info[0] != prev_rx_control_char && !default_freqs_set && ECR->CE_metrics.control_valid) {
    ...
    if (ECR->CE_metrics.control_valid && 'f' == (char)ECR->CE_metrics.control_info[0]) {  
      ...
      node2_control_info_default_set = false;     // Node 2's control info is now 's'.
    }

    // Reset Node 2's info back to 'r'. Node 1 is still 's'.
    else if (ECR->CE_metrics.control_valid && 's' == (char)ECR->CE_metrics.control_info[0]) {
      if (!node2_control_info_default_set) {
        ...
        node2_control_info_default_set = true;      // Node 2's info is now back to 'r'
    ...
    // If receive an 'r', reset Node 2's info back to 'r'
  ...
  // Verify that the default control info has been set after changing to default frequencies.
    ...
    if (!fec0_reset) {
      ...
      // Get the new Tx gain step from Node 2
...
void CE_Sharc21_Node_1::TimeoutCrap(ExtensibleCognitiveRadio *ECR) {
...
void CE_Sharc21_Node_1::FindLowestEP(ExtensibleCognitiveRadio *ECR) {
...
void CE_Sharc21_Node_1::ValidCheck(ExtensibleCognitiveRadio *ECR) {
...
void CE_Sharc21_Node_1::ListenToChannel(ExtensibleCognitiveRadio *ECR) {
  ...
  if (timer_toc(listen_timer) >= listen_time_s && packet_counter < packet_threshold && !go_to_default) {
    SendIntentToChangeTheDangFreq(ECR);
    listening = false;  
    
  } else if (timer_toc(listen_timer) >= listen_time_s && packet_counter >= packet_threshold && !go_to_default) {
...
void CE_Sharc21_Node_1::GoToDefault(ExtensibleCognitiveRadio *ECR) {
  
  // Send Node 2 the default Tx frequency.
